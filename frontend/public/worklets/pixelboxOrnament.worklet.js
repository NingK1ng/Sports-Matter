...
export class PixelBoxOrnament {
    ...
    paint(ctx, size, props) {
        ...
        ctx.fillRect(cornerSize * blockSize, cornerSize * blockSize, width - 2 * cornerSize * blockSize, height - 2 * cornerSize * blockSize);
        // 边框绘制
        const drawBorder = (ctx, dx, dy, flipX, flipY) => {
            ctx.save();
            ctx.translate(dx, dy);
        ...
        const clearCorner = (ctx, dx, dy, size) => {
        ...
        // 实际角落绘制大小
        const drawCorner = (ctx, dx, dy, flipX, flipY) => {
            ctx.save();
            ctx.translate(dx, dy);
            ctx.scale(flipX ? -1 : 1, flipY ? -1 : 1);
            ctx.fillStyle = mainColor;
