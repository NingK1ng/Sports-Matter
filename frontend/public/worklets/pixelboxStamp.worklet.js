...
export class PixelBoxStamp {
    ...
    paint(ctx, size, props) {
        ...
        ctx.fillRect(2 * blockSize, 2 * blockSize, width - 5 * blockSize, height - 4 * blockSize);
        // 水平方向绘制
        const drawHStamp = (ctx, dx, dy, flipY) => {
            ctx.save();
            ctx.translate(dx, dy);
            ...
            for (let i = 0; i < hStampCount; i++) {
                ...
                ctx.clearRect((cornerSize - 1) * blockSize + offset, 0, blockSize, blockSize);
                // 阴影块
                ctx.fillStyle = shadowColor;
                ctx.fillRect((cornerSize - 1) * blockSize + offset, 0, blockSize, blockSize);
                ctx.fillRect(cornerSize * blockSize + offset, blockSize, blockSize, blockSize);
            }
            ctx.restore();
        };
        ...
        const drawLeftStamp = (ctx, dx, dy) => {
            ...
            for (let i = 0; i < vStampCount; i++) {
                ...
                ctx.clearRect(0, (cornerSize - 1) * blockSize + offset, blockSize, blockSize);
            }
            ctx.restore();
        };
        drawLeftStamp(ctx, 0, 2 * blockSize);
        // 右边框绘制
        const drawRightStamp = (ctx, dx, dy) => {
            ctx.save();
            ...
            for (let i = 0; i < vStampCount; i++) {
                ...
                // 主体色块（靠右绘制）
                ctx.fillStyle = mainColor;
                ctx.fillRect(-2 * blockSize, offset, 2 * blockSize, cornerSize * blockSize);
                // 镂空中间小块
                ctx.clearRect(-2 * blockSize, offset, blockSize, blockSize);
                ctx.clearRect(-2 * blockSize, (cornerSize - 1) * blockSize + offset, blockSize, blockSize);
                // 阴影块
                ctx.fillStyle = shadowColor;
                ctx.fillRect(-blockSize, offset - blockSize, blockSize, blockSize);
                ctx.fillRect(-2 * blockSize, offset, blockSize, blockSize);
                ctx.fillRect(-2 * blockSize, (cornerSize - 1) * blockSize + offset, blockSize, blockSize);
                ctx.fillRect(-3 * blockSize, offset + blockSize, blockSize, (cornerSize - 2) * blockSize);
            }
            ctx.restore();
        };
        ...
        ctx.clearRect(width - clearSize - blockSize, 0, clearSize + blockSize, clearSize);
        ctx.clearRect(0, height - clearSize - blockSize, clearSize, clearSize + blockSize);
        ctx.clearRect(width - clearSize - blockSize, height - clearSize - blockSize, clearSize + blockSize, clearSize + blockSize);
        // 内容区域
        ctx.fillStyle = contentColor;
        ctx.fillRect(stampSize + gapBlock * blockSize, stampSize + gapBlock * blockSize, width - blockSize - 2 * (stampSize + gapBlock * blockSize), height - 2 * (stampSize + gapBlock * blockSize));
        // 抠细节
        ctx.fillStyle = shadowColor;
