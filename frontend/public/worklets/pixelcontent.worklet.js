...
export class PixelContent {
    ...
    paint(ctx, size, props) {
        ...
        ctx.lineTo(size.width - cornerSize - halfBorder, size.height - cornerSize - halfBorder);
        // bottom
        ctx.moveTo(cornerSize + halfBorder, size.height - borderShadow - halfBorder - border);
        ctx.lineTo(size.width - cornerSize - halfBorder, size.height - borderShadow - halfBorder - border);
        ctx.stroke();
        ctx.closePath();
        // 绘制阴影
        ctx.fillStyle = bgShadowColor;
        // top
        ctx.fillRect(cornerSize + border, borderShadow, size.width - cornerSize * 2 - border * 2, borderShadow);
        // right
        ctx.fillRect(size.width - cornerSize - border + borderShadow, 0, cornerSize + border - 2 * borderShadow, size.height - border * 2);
        // left
        ctx.fillRect(borderShadow, 0, cornerSize + border - 2 * borderShadow, size.height - border * 2);
        // bottom
        ctx.fillRect(borderShadow, size.height - border * 2, size.width - 2 * borderShadow, borderShadow / 2);
        ctx.fillRect(cornerSize + borderShadow, size.height - 2 * border, size.width - 2 * cornerSize - 2 * borderShadow, 2 * border - borderShadow);
        // content 区域填充色
        ctx.fillStyle = bgColor;
        ctx.fillRect(cornerSize + border, 2 * borderShadow, size.width - 2 * cornerSize - 2 * border, size.height - 2 * border - 3 * borderShadow);
    }
}
